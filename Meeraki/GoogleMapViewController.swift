 ...
 */
@objc protocol GoogleMapViewControllerDelegate: NSObjectProtocol
{
    /**
...
class GoogleMapViewController: UIViewController, GMSMapViewDelegate
{
    ...
    /// Marker object to display on source location
    var sourceMarker : CustomMarker!
    /// Marker object to display on destination location
    var destinationMarker : CustomMarker!
    /// Boolean value to allow map drag
    var allowMapDrag : Bool!    = true
    /// Store padding value
    var padding : CGFloat!
    /// Boolean value to display current location
    var showCurrentLoc : Bool!    = true
    /// Boolean value to display current location button
    var showCurrentLocBTN : Bool! = false
    
    //MARK:- Variable Declerations - Getters
    /// Store ETA model data
    var etaModelFetched             = ETAModel()
    /// Store source text to search
    var sourceSearchText            = String()
    /// Store destination text to search
    var destinationSearchText       = String()
    /// Map view object
    var customMapView               = GMSMapView()
    
    //MARK:- Variable Declerations - ETA
    /// Store total duration in seconds to reach any location
    ...
    /// Delegate obj to access protocol methods
    weak var delegate: GoogleMapViewControllerDelegate?
    /// To draw polyline between source to destination
    ...
    /// Check if multiple marker plot at same location, it give minor variation between two markers.
    ///
    /// - Parameters:
    ///   - latitude: The latutude
    ///   - longitude: The longitude
    /// - Returns: The variable coordinate
    ...
    {
        let key : String = "AIzaSyBh4Px06vNGuFRY1ClNDkJZW65CKSzCdi8" //Macros.ApiKeys.mapKeyGoogle
                            ...
                            if routesArray.count > 0
                            {
                                ...
                                if routesArray.count > 0
                                {
                                    ...
                                    self.calculateETA(routesN: routeDic , completionHandler: { (eta) in
                                        ...
                                        self.animateRoute(self.polyline, origin: origin.coordinate, destination: destination.coordinate, pathColor: UIColor.blue, zoomCamera: zoomCamera)
    ...
    /// Place marker at coordinate with user name
    ///
    /// - Parameters:
    ///   - marker: CustomMarker object
    ///   - mapView: GMSMapView object
    ///   - name: User name to display
    ...
    /// To place parking marker at perticular location
    ///
    /// - Parameters:
    ///   - marker: The marker obj
    ///   - mapView: The mapview obj
    ///   - center: Boolean value to place marker at center or not
    ...
    /// Get accurate distance between source and destination
    ///
    /// - Parameters:
    ...
    /// Get Map point from coordinate
    ///
    /// - Parameter cordinate: Coordinate to get point
    /// - Returns: Map point
    ...
    func takeCameraToMarker(_ marker : GMSMarker)
    {
        ...
        customMapView .animate(with: GMSCameraUpdate .setTarget(marker.position, zoom:16))
    ...
    /// Zoom camera to coordinate
    ///
    /// - Parameters:
    ///   - coordinate: Coordinate where to zoom
    ///   - withZoom: Zoom value
...
//    }
    ...
    /// Remove geofence area from google map
    ///
    /// - Parameter modelObj: Organization model obj
...
//    }
    ...
    /// Called when tap on loacation button
    ///
    /// - Parameter mapView: GMSMapView object
    /// - Returns: true/false
    func didTapMyLocationButton(for mapView: GMSMapView) -> Bool
    {
        if  MPUserDefaults.shared.getUserLastCurrentLocationLat() == nil
        {
            return false
        }
        
        let latRetrv : Double = Double(MPUserDefaults.shared.getUserLastCurrentLocationLat())!
        
        let longRetrv : Double = Double(MPUserDefaults.shared.getUserLastCurrentLocationLong())!
        ...
        let lat : Double = Double(MPUserDefaults.shared.getUserLastCurrentLocationLat())!
        
        let long : Double = Double(MPUserDefaults.shared.getUserLastCurrentLocationLong())!
...
class MPUserDefaults: NSObject
{
    /// shared instance
    static let shared = MPUserDefaults()
    /// standard default variable
    let standardDefault = UserDefaults.standard
    
    //MARK:- Get and Set user last current location
    /**
     To get user last current location lat from userDefaults.
     */
    func getUserLastCurrentLocationLat() -> String!
    {
        return  ((standardDefault .value(forKey: "userLastLocationLat") as? String) != nil) ? (standardDefault .value(forKey: "userLastLocationLat") as? String)! : nil
    }
    
    /**
     To get user last current location long from userDefaults.
     */
    func getUserLastCurrentLocationLong() -> String!
    {
        return  ((standardDefault .value(forKey: "userLastLocationLong") as? String) != nil) ? (standardDefault .value(forKey: "userLastLocationLong") as? String)! : nil
    }
    
    /**
     To set user last current location lat & long in userDefaults.
     */
    func setUserLastCurrentLocation(_ lat : String, long : String)
    {
        standardDefault.setValue(lat, forKey: "userLastLocationLat")
        standardDefault.setValue(long, forKey: "userLastLocationLong")
    }
}
